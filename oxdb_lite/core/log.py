...
from oxdb_lite.utils.dp import (
    UIDX,
...
class Oxdb:
    ...
    def _db_path_validator(
        ...
        """
        Validates and potentially updates the database path, appending the '.oxdb_lite' extension if missing.
        ...
        if db_path:
            ...
            db = dir_name.split(config.settings.OXDB_EXT)[0] if dir_name.endswith(config.settings.OXDB_EXT) else dir_name
            final_path = db_path if dir_name.endswith(config.settings.OXDB_EXT) else db_path + config.settings.OXDB_EXT
        elif db:
            db = db.split(config.settings.OXDB_EXT)[0] if db.endswith(config.settings.OXDB_EXT) else db
            final_path = os.path.join(os.path.expanduser("~"), config.settings.BASE_DB_COLLECTION, db + config.settings.OXDB_EXT)
        else:
            db, final_path = (
                "",
                os.path.join(os.path.expanduser("~"), config.settings.BASE_DB_COLLECTION, config.settings.OXDB_EXT),
            )
    ...
    def get_dbs(self) -> List[str]:
        ...
        """
        db_root_path = os.path.join(os.path.expanduser("~"), config.settings.BASE_DB_COLLECTION)
...
class dbDoc:
    ...
    def _load_oxdld(self, oxd_doc_name: str,data_encoading:str=config.settings.DBIN_METHOD) -> Oxdld:
        """
        ...
        return Oxdld(oxd_doc_path,data_encoading)
    ...
    def search(
        ...
        query: str,
        topn: int = 10,
        by: Optional[str] = config.settings.SIM_FORMAT,
        idx: idxdata = None,
        uid: Optional[str] = None,
        time: Optional[str] = None,
        date: Optional[str] = None,
        ...
        if by not in config.settings.SIM_FORMATS:
            raise ValueError(
                f"Invalid search method '{by}'. Must be one of {config.settings.SIM_FORMATS}."
            )
